...
class Eventbrite_Just_Announced_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if ( ! empty( $recent_events ) ) :
			...
			<ul>
			<?php foreach ( $recent_events as $recent_event ) : $recent_event = $recent_event->event; $wp_event_url = eb_get_wp_event_url( $recent_event );?>
				<li>
					<?php if ( $wp_event_url ) : ?>
						<a href="<?php echo esc_url( $wp_event_url ); ?>" title="<?php echo esc_attr( $recent_event->title ); ?>">
					<?php endif; ?>
					<?php echo esc_html( $recent_event->title ); ?>
					<?php if ( $wp_event_url ) : ?>
						</a>
					<?php endif; ?><br/>
					<span><?php echo esc_html( mysql2date( get_option( 'date_format' ), $recent_event->start_date ) ); ?></span>
				</li>
			<?php endforeach; ?>
			</ul>
